...
! model with z,m for critical layer demonstration
...
subroutine set_initial_conditions
 ...
 implicit none
 integer :: i,j,k,l,n,m
 real*8 :: om,Jac,kh2,mstar,funcA,funcB,na,Lm,r
 real*8, parameter :: E0=3d-3, nb = 2./pi, cstar = 0.5
 real*8 :: z, N_func
 N_func(z) = z*0+30*f0
 ...
 do k=zs_pe-onx,ze_pe+onx 
  do j=ys_pe-onx,ye_pe+onx   
   do i=xs_pe-onx,xe_pe+onx
     ...
     U(i,j,k) =  0.40*(1+tanh( (zt(k)+750)/150))/(2.)
    ...
    do k=zs_pe-onx,ze_pe+onx 
     do j=ys_pe-onx,ye_pe +onx  
      do i=xs_pe-onx,xe_pe+onx 
        ...
        if (mt(m)<0) E(i,j,k,l,n,m) = 1.+sin(mt(m)*2*pi/(Lm/2/3.) ) * sin(zt(k)*2*pi/(Lz/3.))
